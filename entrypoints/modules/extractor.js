...
const setupFunctions = {
  setup_top_names: (source, vector_size, args) => {
    top_names = {};
    getExtensionFile(browser.runtime.getURL(source), 'text', (resp) => {
...
export const extractFeatures = function(cookieDat) {
  ...
  // );
  let max_updates = Math.min(1, cookieDat['variable_data'].length);
  let max_diffs = Math.min(1, cookieDat['variable_data'].length - 1);
